...
cdef mpc BubMaster(mpfr k2, mpc M1, mpc M2):	
	...
	if (1j*(m1-m2-1)+2*sqrt(m1)).imag > 0 and (1j*(1 + m1-m2)+2*sqrt(m2)).imag < 0:
		sign = 1
	else:
		sign = 0
	bubmaster = SQRT_PI/sqrt(k2)*(1j*log(1j*( m1-m2-1)+2*sqrt(m1))-1j*log(1j*(1 + m1-m2)+2*sqrt(m2))+2*PI*sign)
	return bubmaster
